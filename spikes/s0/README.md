...
python3 spikes/s0/real_cli_probe.py --output docs/spikes/s0-real-cli-results.json
```
...
结果写入 `docs/spikes/`。安全实验的 `boundary` 场景有意验证仍可泄露的路径，**expectation_met 不等于防护成功**。有失败/泄露是 S0 的重要结果，不应删掉这些测试把通过率做高。
...
python3 spikes/s0/real_cli_probe.py --runtime-dir "$laodi_runtime_dir" --output docs/spikes/s0-real-cli-runtime-results.json
```
...
后续非阻断Go开发版及ZCode静态契约验证见[开发版使用说明](../../docs/DEVELOPMENT.md)；S0阻断探针保留作研究，不是当前V1默认功能。

公开材料说明：历史结果中的合成CLI线程标识、随机运行目录已脱敏；`docs/spikes/s0-runtime-package-lock.json`是脱敏取证记录，不应直接当安装锁文件使用。
