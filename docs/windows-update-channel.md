...
beta.2 及更早二进制不会自动获得新代码。beta.2 首次升级需显式指定 `--version v0.4.1-windows.beta.4`；旧版安装未完成时，重新运行[命令行安装入口](WINDOWS.md#命令行安装)。只有公开资产校验完成后才推广推荐通道。新发行版若比通道推荐更高，默认更新会拒绝降级，直到维护者推广通道。
