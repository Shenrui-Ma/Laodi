...
适用版本：`0.4.1-beta.1`。支持工具访问请求和敏感输出检测，包含两种工具协议适配。**不代表覆盖两款产品的全部后台行为，也不确认内容已发送。** 纯内存直传检测另列未来TODO。
...
核心二进制与无窗口通知 helper 的构建沿用[开发说明](DEVELOPMENT.md)。运行中的旧版后台需按更新说明处理。

高级配置先运行 `./bin/laodi hooks --help`，从已安装版本列出的有效适配标识中选择目标，将其设为 shell 变量 `laodi_adapter`。该变量必须由用户设置，不是产品名或示例占位值。

```sh
# 设置 laodi_adapter 后，先预览再应用：
./bin/laodi hooks install --adapter "${laodi_adapter:?请先选择有效适配标识}"
./bin/laodi hooks install --adapter "$laodi_adapter" --apply
```

程序写入所选客户端的用户配置目录，保留其他字段和 Hook；重复安装不叠加。只用新会话验证接入，当前长任务不重启。Hook 配置成功不等于真实客户端已经发来事件。

**需要运行监测器消费事件。** 某APP快照与工具事件可共用一个进程：
...
`--hooks-only`不要求安装某APP。两种监测模式使用不同根身份，不能对同一已初始化状态目录偷偷切换；需保持模式一致或显式使用不同`--state-dir`，对应Hook也须使用同一目录。已安装服务的参数变化仍按原所有权规则拒绝静默重启。
...
```sh
./bin/laodi hooks uninstall --adapter "${laodi_adapter:?请先选择有效适配标识}" --apply
```

保护边界：配置符号链接、重复JSON键、JSONC、超限配置及不明确的归属不自动修复。适配器 A 已有其他禁用 Hook 时不会擅自开启它们；适配器 B 的全局禁用也不被覆盖。配置和收据各自原子写入，但不是跨文件事务，异常中断可能留下需要检查的不完整安装；外部同时编辑设置的竞态不能完全消除。
...
适配器 A/B 按各自的官方工具事件协议实现，字段和配置方式分别处理。安装器默认匹配 Bash 和 Read；Grep、MCP、自定义工具、宿主索引和后台上传不因此被覆盖。适配器 A 还兼容 read_file 输入，但默认配置未启用该别名。

```sh
go test -race ./...
go vet ./...
python3 scripts/dev/verify_tool_hooks.py \
  --binary bin/laodi --output .omx/experiments/v02-tool-hook-results.json
```

回放使用临时HOME，实际安装/卸载合成配置，执行生成的shell Hook命令与Go监测器，检查四路并发、正常反例、截断、失败输入、重启、秘密不落盘及fake通知helper。它不会启动真实Agent、调用模型或发送系统通知。真实适配器 A/B回调触发、宿主取消时的行为与系统横幅送达仍须单列验收，不能将协议回放等同实机完整通过。

具体实验结果和构建测量记录保留在本地，不随源码发布。回放验证仅覆盖所列输入与临时配置，不能替代真实客户端的完整验收。
