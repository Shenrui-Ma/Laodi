...
4. Windows 可用 `laodi notifications status` 只读查询通知；`protect` 目前仅适用于 macOS，不要把 Windows 工具监测说成已具备归档阻断。优先回复实际发现、覆盖状态与用户是否需要处理。必要时读取 [使用说明](references/usage.md)。
...
- 用户明确要求测试 macOS 打包保护或查看测试提醒时，可运行 `laodi protect test`，需要提醒时加 `--notify`。先核对已安装版本支持该命令；测试失败不发送成功通知。结果仅证明主动自检，不是某 APP 的真实拦截事件，不计入泄露或阻断次数。
