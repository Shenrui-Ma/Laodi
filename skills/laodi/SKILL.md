...
1. 用户询问状态或提醒时，先通过宿主的正常命令工具检查 `command -v laodi`。若找不到，检查 macOS 官方安装位置 `"$HOME/Library/Application Support/Laodi-skills/runtime/laodi"` 是否可执行，并用该完整路径调用。发行包不修改 PATH，不能仅凭第一步找不到就断言未安装。两个位置均不可用时，再查看可信工作区的 README 安装说明；不要自行遍历家目录、编造安装地址或自动使用 `sudo`。下表的 `laodi` 代表已确认的可执行文件。
2. 按用户问题执行对应的只读查询：
