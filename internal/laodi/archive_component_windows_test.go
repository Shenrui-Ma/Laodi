...
func TestWindowsArchiveOriginalProducerABA(t *testing.T) {
	...
	runGit := func(args ...string) {
		...
		c.Dir = repo
		c.Env = append(os.Environ(), "GIT_CONFIG_NOSYSTEM=1", "GIT_CONFIG_GLOBAL="+filepath.Join(f.home, "absent-gitconfig"))
	...
	t.Log("original producer: baseline received with Git history and extra config; 3 pending retries + existing/new workspace denied; identical pending ciphertext and increment received after restore")
}
